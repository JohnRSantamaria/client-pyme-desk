 ...
 * 	const [estadoFilter, setEstadoFilter] = useState<Estado>('');
	...
	const handleUpdatePedido = useCallback(
		async ({ id, estadoFilter }: { id: number | undefined; estadoFilter: Estado }) => {
			const result = await updatePedido({ id, estado: estadoFilter });
	...
	useEffect(() => {
		if (estadoFilter) {
			Swal.fire({
				title: '¿Está seguro?',
				text: `El estado de la orden número ${id} cambiará a "${estadoFilter}".`,
				icon: 'warning',
				showCancelButton: true,
				confirmButtonColor: CONFIRM_BUTTON_COLOR,
				cancelButtonColor: CANCEL_BUTTON_COLOR,
				...
				if (result.isConfirmed) {
					handleUpdatePedido({ id, estadoFilter });
				}
			});
		}
